...
if [ -n "${RELEASEPANEL_SSH_IDENTITY_FILE:-}" ] && [[ "${RELEASEPANEL_SSH_IDENTITY_FILE}" == /root/* ]]; then
    ...
    if [ -n "${RELEASEPANEL_DEPLOY_COMMIT_SHA:-}" ]; then
        ...
        git -C "${release}" checkout --force "origin/${RELEASEPANEL_BRANCH}"
    fi
    ...
    if [ -n "${RELEASEPANEL_DEPLOY_COMMIT_SHA:-}" ]; then
        ...
        run_as_app_user_in "${release}" git checkout --force "origin/${RELEASEPANEL_BRANCH}"
    fi
fi
...
if [ "${first_deploy}" = "true" ]; then
    ...
    if ! RELEASEPANEL_DEPLOY_ENV="${RELEASEPANEL_DEPLOY_ENV}" "${smoke_script}" "${smoke_args[@]}"; then
        smoke_status=1
    fi
fi
