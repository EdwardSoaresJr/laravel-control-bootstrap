...
const host = process.env.RELEASEPANEL_RUNNER_HOST || '127.0.0.1';
const port = Number.parseInt(process.env.RELEASEPANEL_RUNNER_PORT || '9000', 10);
const apiKey = process.env.RELEASEPANEL_RUNNER_KEY || '';
const logPath = process.env.RELEASEPANEL_RUNNER_LOG || '/var/log/releasepanel-runner.log';
const toolkitPath = path.resolve(__dirname, '..');
const normalTimeoutMs = Number.parseInt(process.env.RELEASEPANEL_RUNNER_NORMAL_TIMEOUT_MS || '120000', 10);
const deployTimeoutMs = Number.parseInt(process.env.RELEASEPANEL_RUNNER_DEPLOY_TIMEOUT_MS || '900000', 10);
const panelUrl = (process.env.RELEASEPANEL_PANEL_URL || '').replace(/\/+$/, '');
const heartbeatIntervalMs = Number.parseInt(process.env.RELEASEPANEL_RUNNER_HEARTBEAT_MS || '30000', 10);
...
const actionAliases = {
    'deploy-no-route-cache': 'deployNoRouteCache',
    'deploy-strict': 'deployStrict',
};
...
const siteActions = {
    ...
    deployNoRouteCache: {
        method: 'POST',
        route: '/deploy/:site/:env/no-route-cache',
        args: (site, env) => siteCommandArgs('deployNoRouteCache', site, env),
        timeoutMs: deployTimeoutMs,
        locked: true,
    },
    deployStrict: {
        method: 'POST',
        route: '/deploy/:site/:env/strict',
        args: (site, env) => siteCommandArgs('deployStrict', site, env),
        timeoutMs: deployTimeoutMs,
        locked: true,
    },
    rollback: {
...
if (!apiKey || apiKey === 'CHANGE_ME') {
    console.error('RELEASEPANEL_RUNNER_KEY must be set in runner/.env before starting the runner.');
    process.exit(1);
}
...
async function sendHeartbeat() {
    ...
    try {
        const response = await fetch(`${panelUrl}/api/runner-heartbeat`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                runner_url: process.env.RELEASEPANEL_RUNNER_PUBLIC_URL || `http://${host}:${port}`,
...
function requireApiKey(request, response, next) {
    if (request.get('X-RELEASEPANEL-KEY') !== apiKey) {
        appendLog({
...
function siteCommandArgs(actionName, site, env) {
    const generic = {
        ...
        deployNoRouteCache: 'site-deploy.sh',
        deployStrict: 'site-deploy.sh',
    ...
    if (actionName === 'deployNoRouteCache') {
        args.push('--skip-route-cache');
    }

    if (actionName === 'deployStrict') {
        args.push('--strict');
    }
...
function certificateSummary(env) {
    ...
    const certPath = `/etc/letsencrypt/live/${serverName}/fullchain.pem`;
...
function actionDefinition(actionName) {
    ...
    return {
        ...
        definition: actions[resolvedAction],
    };
}
...
function startActionRun(actionName, definition, request, response) {
    ...
    if (actionName === 'refreshReleasePanel' && env !== 'releasepanel') {
        response.status(404).json({
            success: false,
            message: 'ReleasePanel self-update is only available for the ReleasePanel environment.',
        });
        return;
    }
...
app.listen(port, host, () => {
    appendLog({
        ...
        success: true,
        duration_ms: 0,
        message: `ReleasePanel runner listening on ${host}:${port}`,
    });
