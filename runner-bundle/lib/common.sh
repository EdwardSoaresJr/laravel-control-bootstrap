...
local_http_check() {
    ...
    health_code="$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 -H "Host: ${RELEASEPANEL_SERVER_NAME}" http://127.0.0.1/health || true)"
    ...
    if [ "${health_code}" != "404" ]; then
        if [ "${health_code}" = "000" ]; then
            ...
            echo "[warning] health check failed (${health_code})"
        fi
    ...
    if [ "${root_code}" = "000" ]; then
        ...
        echo "[warning] /health not found; fallback / returned ${root_code}"
    fi
...
local_https_check() {
    local strict="${1:-false}"
    local cert="/etc/letsencrypt/live/${RELEASEPANEL_SERVER_NAME}/fullchain.pem"
    ...
    if [ ! -f "${cert}" ]; then
        echo "[warning] HTTPS check skipped; certificate missing at ${cert}."
        [ "${strict}" = "true" ] && return 1
        return 0
    fi
...
detect_and_run_asset_build() {
    ...
    if [ -f "${release}/package-lock.json" ]; then
        ...
        run_as_app_user_in "${release}" npm ci
    else
        log "Installing npm dependencies with npm install."
        run_as_app_user_in "${release}" npm install
    fi
